"""
T5 (mT5) ile Sentetik Teknoloji Haber Başlığı Üretimi
- Model: google/mt5-base (Türkçe destekli)
...
CSV_OUTPUT = 't5_sentetik_teknoloji_haberleri_3000.csv'
TARGET_SENTENCES = 3000
MODEL_NAME = 'google/mt5-base'  # Google mT5-base (101 dil, akademik standart)
...
print(f"[INFO] Türkçe T5 modeli yukleniyor: {MODEL_NAME}")
from transformers import AutoTokenizer, AutoModelForSeq2SeqLM
tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
model = AutoModelForSeq2SeqLM.from_pretrained(MODEL_NAME).to(device)
model.eval()
print("[INFO] Türkçe T5 modeli hazir")
...
def uret_3000_cumle(orijinal_cumleler, target=3000):
    ...
    attempts = 0
    max_attempts = target * 10  # Sonsuz döngü önleme
...
def metrik_analizi(orijinal_cumleler, sentetik_cumleler):
    ...
    print("\n" + "="*60)
    print("5 TEMEL METRİK ANALİZİ (T5)")
    print("="*60)
